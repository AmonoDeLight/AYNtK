	...
	function user_message(sUser){
		...
		var o={
			"81062b9b-9562-6aa7-a0d0-a62f49768aad": {
				text: "Совет. Если хочется сохранить карточки, нажмите [Ctrl]+[P] или выберете 'Печать' через меню браузера и там выберите печать в PDF. В полученном файле будут карточки.<br>Удобнее всего это делать через Chrome - там наглядно показывается, каким получится файл.<br><br> Присоздании сообщения полезно указывать e-mail, на который можно прислать ответ."
			}
		}
		if(o[sUser]) {
			show(o[sUser].text);
	...
	function show_notes(){
		let aNotes = document.querySelectorAll('.note');
		let oNow = new Date();
		for (let i=0; i< aNotes.length; i++) {
			let sStart = aNotes[i].dataset.start;
			let sEnd = aNotes[i].dataset.end;
			let oStart, oEnd;
			
			let bVisible = true;
			
			if(sStart) {
				oStart = new Date(sStart);
				if(oNow<oStart) {
					bVisible = false;
				}
			}
			...
			aNotes[i].style.display = bVisible? 'block' : 'none';
